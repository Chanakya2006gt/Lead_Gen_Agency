...
:root {
  /* Canvas & Backgrounds */
  --bg-canvas: #080A0F;        /* Deep matte void, zero eye-strain */
...
* **Display & Body**: `Inter` or `Geist Sans` (`font-sans`), `-webkit-font-smoothing: antialiased`.
* **Quantitative Data & Code**: `Geist Mono` or `JetBrains Mono` (`font-mono`) with `tabular-nums`.
...
* **Left**: Engine identity icon + title (`LEAD ENGINE`) + environment chip (`v2.4 Production`).
* **Right**: Real-time stats summary (`Total Discovered`, `Qualified`, `Opportunities`) in monospace + `Export CSV` button.
...
* **Inputs**:
  ...
  * Category / Niche Input with instant keyboard focus (`Cmd+K`).
  * Radius selector (10km / 15km / 25km).
  * `Run Discovery Scan` button with active pulse spinner.
* **Dynamic Market Chips**: Auto-detects operator timezone/GPS and suggests 4 1-click presets (e.g. *Dental Clinics*, *Cosmetic Surgery*, *Commercial HVAC*, *High-End Salons*).

### 5.3 The Lead Matrix Table (`LeadMatrixTable.tsx`)
...
### 5.4 Sales Intelligence Dossier Modal (`LeadDossierModal.tsx`)
Structured like a high-conviction deal memo:

1. **Header**: Business Name + Category + Direct Maps Link + Phone Number.
2. **Section 1: WHY THIS LEAD**: 3 bulleted commercial arguments explaining why this business has demand, purchasing power, and an urgent gap.
...
4. **Section 3: RECOMMENDED APPROACH & SCOPE**:
   * Estimated Value Range (e.g. `$2,500 – $5,000`).
   * Suggested Technical Scope summary.
   * Multi-Channel Outreach Tabs with 1-click clipboard copy:
     * **Cold Email Deck**: Tailored subject line, exact identified gap, proposed scope, teardown video offer.
     * **WhatsApp Instant Hook**: 3-line punchy hook with direct `wa.me` 1-click launch button.
...
5. **Footer Triage Workflow**: `Mark Reviewed`, `Ready for Outreach` (Emerald CTA), `Archive`.
...
* **Delta Badges**:
  * `+14 reviews gained`: Emerald pill with positive upward arrow.
  * `★ 4.9`: High-contrast amber star with bold white score.
...
* **Mobile (375px–480px)**: Fast triage view. Cards replace wide table rows, swipeable outreach scripts, direct phone call triggers.

---

## 8. Accessibility & Compliance Standards

* **WCAG 2.2 AA Contrast Ratio**: All body copy meets $\ge 4.5:1$ contrast against dark surfaces; all quantitative mono badges meet $\ge 3:1$.
* **Keyboard Navigation**: `Tab` cycles through leads, `Enter` opens dossier, `Escape` closes modals, `Cmd+K` focuses discovery search bar.
* **Screen Reader Support**: Semantic HTML tables (`<thead>`, `<tbody>`, `<th>`, `aria-label` attributes on score badges and actions).
...
src/
├── app/
│   ├── globals.css              # Dark terminal CSS variables, clean card utility classes
...
│   ├── LeadMatrixTable.tsx      # Central Opportunity Matrix Table
│   ├── LeadDossierModal.tsx     # High-Conviction Sales Intelligence Memo
│   ├── ScoreGauge.tsx           # Monospace Quantitative Score Indicator
│   ├── LiveTerminal.tsx         # Collapsible Background Telemetry Stream
│   └── DashboardClient.tsx      # State Coordinator & Opportunity Workspace
```
